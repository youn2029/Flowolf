...
<section class="content full-calendar">
	<div class="project-wrap">
		...
		<div class="col-xs-12 col-sm-12 col-md-10 col-lg-10 padleft-0 full-calendar-left">
			<strong class="dis-block marbtm-30 size-18 color-black">일정 : ${proVo.pro_name}</strong>
			<div id='calendar'></div>
		</div>
...
function placeFormatter(r){
	...
	var diff=null;
	var placeMaker = '';
	if(r.place==diff|| r.place==placeMaker || r.place== null|| r.place =='undefined'){
		$("#viewResolver").hide();
	}else{
		$("#viewResolver").show();
		placeMaker = '&nbsp &nbsp'+r.place+'<a href="'+"https://maps.google.com?q="+r.place+'" target="google_blank" class="marleft-15">'+'지도보기'+'</a>'+'<p></p>';
	}
	return placeMaker;
}
