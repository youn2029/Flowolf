...
public class ScheduleController {
	...
	public String insertSchedule(Model model, @RequestParam("alert_time") String alert_time,
	...
	public String updateSchd( @RequestParam("alert_time") String alert_time,ScheduleVO vo,
		...
		ScheduleVO scvo = schService.getSchdDetail(vo.getSchd_no());
		//vo객체 일부내용 수정
		vo.setSchd_start_time(time[0]);
		vo.setSchd_end_time(time[1]);
		vo.setSchd_alarm(notificationTimer);
		vo.setSchd_del_chk(scvo.getSchd_del_chk());
		vo.setSchd_fix_chk(scvo.getSchd_fix_chk());
		vo.setSchd_fix_date(scvo.getSchd_fix_date());
	...
	public ModelAndView testAjax(Model model,HttpSession session,
		...
		System.out.println("현재시간? : " + Format.notificationFormatter(Format.timeFormat(new Date()),0));
		System.out.println("알림갯수 : "+ schdvo.size());
		mv.addObject("schdVo", schdvo);
